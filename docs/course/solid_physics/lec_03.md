...
质量 $m$ 越大，抛物线 $E(k)$ 越宽（开口越大）。

波函数为平面波 $\psi_k(\vec{r})=\dfrac{1}{\sqrt{V}}\exp(i\vec{k}\cdot\vec{r})$ ，已归一化。

波函数的模平方即为概率密度。对于自由电子，其概率密度为定值 $\dfrac{1}{V}$ ，说明电子的位置分布是全空间均匀的，完全不确定，但是其动量 $\vec{p}=\hbar\vec{k}$ 是定值，完全确定，满足不确定性原理。
...
$\vec{k}$ 空间点阵密度为其倒数 $\rho(\vec{k})=\dfrac{V}{8\pi^3}$ ，考虑电子自旋简并度为 2，单位 $\vec{k}$ 空间本征态数目，即 $\vec{k}$ 空间的能态密度为 $g(\vec{k})=2\rho(\vec{k})=\dfrac{V}{4\pi^3}$ .

能量相等的状态，对应的波矢 $\vec{k}$ 的模相等。因此能量 $E$ 以内的所有波矢状态围成一个球体，球体内电子能态总数为 $Z(E)=2\cdot \dfrac{V}{8\pi^3} \cdot \dfrac43\pi|\vec{k}|^3=2\cdot \dfrac{V}{8\pi^3} \cdot \dfrac43\pi\left(\dfrac{\sqrt{2mE}}{\hbar}\right)^3$ .
...
$k$ 空间的平移对称性：布洛赫波的波矢 $k$ 平移 $\dfrac{2\pi n}{a}$ 之后，平面波部分不变，只是调幅因子变化。但是 $k'=k+\dfrac{2n\pi}{a}$ 的一组波函数能量不同，取最小的波矢——简约波矢 $k$ 落在第一布里渊区 $-\dfrac{\pi}{a}\sim\dfrac{\pi}{a}$ ，使得同一能带中 $E(k)=E(k+G_n)$ ，不同能带中 $E(k)\ne E(k+G_n)$ .
...
绝对零度 $T=0\,\text{K}$ 时，**费米能量** $E_{F0}$ 是费米能级 $E_F$ 在绝对零度下的特例。取极限得到，费米分布函数在费米能量以下为 $1$ ，费米能量以上为 $0$ ，也就是费米能量是电子填充的最高能级的能量。
...
平均自由程为 $l=v_F\tau$ ，弛豫时间为 $\tau=\dfrac{m}{ne^2\rho}$ ，代表指载流子平均两次散射之间的时间间隔，也就是平均自由时间。 $\tau$ 表达式中各物理量含义为：电子质量、电子浓度、元电荷、电阻率 $\rho$ .

基态下 $T=0\,\text{K}$ ，总能量 $E=\dfrac{3}{5}NE_{F0}$ ，单个电子平均能量 $\dfrac{3}{5}E_{F0}$ .

温度从绝对零度开始升高时，只有费米面附近少部分 $|E-E_{F0}|\sim k_BT$ 的电子激发到较高能态。费米能量 $E_{F0}$ 以下占有概率减小，以上占有概率增大。由于能量越高能态密度越高（三维），因此**有限温度下费米能级略低于费米能量**， $E_F<E_{F0}$ .
