...
1. 选择一个阈值 T 的初始值
2. 用 T 分割图像，获得 2 组像素 $G_1$ （灰度大于 T）和 $G_2$ （灰度小于 T）
3. 计算区域 $G_1,G_2$ 中所有像素平均灰度 $g_1,g_2$
4. 计算新阈值 $T=(g_1+g_2)/2$
5. 重复步骤2至4，直到逐次迭代所得的 $T$ 值之差小于某参数 $T_0$

大津算法（最大类间方差法）。思想：类间方差最大，类内方差最小。判决准则 $\eta(t) = 1-\dfrac{\sigma_\omega^2}{\sigma_T^2}$ ，最佳阈值 $t$ 为（等效于使类间方差最大的阈值）： $t^*=\displaystyle\arg\max_{0\leq t\leq L-1}\eta(t)$ .
...
2. 将彩色像素 $f(x,y)$ 看作 $\mathbb{R}^{3}$ 的向量，直接对彩色向量处理。
