...
> 信号就是一个以时间作为自变量的函数，例如 $x(t)$ ，或一个离散时间序列，例如 $\{x(n)\}$ ，其取值一般是实数或复数。如果是 $N$ 维函数，可表示为矢量函数 $[x_1(t) ,x_2(t) , \cdots , x_N(t)]^T$ ，它在任一时刻都是一个 $N$ 维矢量。
>
...
!!! tip
    由于宽平稳过程的均值为常数，我们通常令 $X(t)-\mu_X$ 为新的 $X(t)$ ，变为零均值，且不改变其他信息，更方便研究。之后我们的讨论的大多都是零均值宽平稳随机过程。

!!! note
    多数宽平稳过程都不是严平稳的，但高斯过程是个例外。**宽平稳的高斯过程一定是严平稳的**，因为高斯过程的任意有限维分布函数仅由均值和协方差函数决定。

    有关高斯过程的讨论将在 [Lecture 2 Gaussian Process 高斯过程](./lec_02.md) 中进行。
...
!!! note
    泊松过程是典型的增量过程，具体讨论参考 [Lecture 3 Poisson Process 泊松过程](./lec_03.md)。
