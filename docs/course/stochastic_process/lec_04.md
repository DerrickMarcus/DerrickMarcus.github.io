...
强度参数不是常数，而是关于时间 $t$ 的函数 $\lambda\to\lambda(t)$ .
...
在复合泊松过程的基础上考虑时间的影响，每个事件发生的贡献会随时间推移而发生变化。设齐次泊松过程 $N(t)\sim\text{Poisson}(\lambda t)$ 各个事件的发生时间为 $S_k,\;k=1,2,\cdots$ ，用 $X_k(t,S_k)$ 表示在 $t$ 时刻第 $k$ 个事件的贡献值，假设所有 $X_k$ 都是**独立同分布的**，即每个事件的贡献随时间的变化相同（方便讨论和分析），可将 $X(t,S_k)$ 视为一个随机过程。令 $Y(t)=\displaystyle\sum_{k=1}^{N(t)}X(t,S_k)$ ，称为过滤泊松过程。
