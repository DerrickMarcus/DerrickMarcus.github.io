...
**其中，离子键没有交换势吸引，共价键没有库伦吸引，混合键4种作用能都有。**原子核（离子实）对电子的库仑吸引是离子键成键的基本原因，原子波函数相互重叠是共价键成键的基本原因。
...
离子晶体，如 $\ce{NaCl,CsCl}$ 。原胞（含有一正一负两个离子）的平均库仑能 $\dfrac{\alpha q^2}{4\pi\epsilon_0 r}$ ， $\alpha$ 为马德隆常数，只和晶格形式有关。典型数值 $\ce{NaCl}:1.747565,\ce{CsCl}:1.762675,\ce{ZnS}:1.6381$ 。
...
原子在形成共价键时可能发生轨道杂化，如金刚石 $sp^3$ 轨道杂化形成4个相同的共价键。杂化：在成键的过程中，由于原子间的相互影响，同一原子中几个能量相近的不同类型的原子轨道(波函数)，可以进行线性组合，重新分配能量和确定空间方向，组成数目相等的新原子轨道。
