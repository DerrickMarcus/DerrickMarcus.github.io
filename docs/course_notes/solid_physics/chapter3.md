...
布洛赫定理：在晶格周期性势场 $V(\vec{r})=V(\vec{r}+\vec{R}_n)$ 中，电子波函数与自由电子波函数相差一个周期性调幅因子，且周期与晶格周期相同。形如 $\psi(\vec{r})=\mathrm{e}^{i\vec{k}\cdot\vec{r}}u(\vec{r})$ ，调幅平面波，称为布洛赫函数。

自由电子波函数 $\mathrm{e}^{i\vec{k}\cdot\vec{r}}$ 慢变，代表晶体中传播的行波，反映晶体在各个原胞之间的共有化运动。调幅因子 $u(\vec{r})$ 快变，周期与晶格周期相同，反映单个原胞中电子的运动。

布洛赫定理的另一种表述：周期性势场中电子波函数满足 $\psi(\vec{r}+\vec{R}_n)=\mathrm{e}^{i\vec{k}\cdot\vec{R}_n}\psi(\vec{r})$ ，平移一个晶格矢量 $\vec{R}_n$ 后波函数增加相位因子。
