...
3. 面心立方晶格：铜，金，银，铝 $\ce{Cu,Au,Ag,Al}$ . 在面心立方的基础上内部加 4 个原子:
    1. 金刚石晶格（内部原子和面上原子相同）：金刚石，硅，锗。
    2. 闪锌矿晶格（内部原子和面上原子不同）：闪锌矿 $\ce{ZnS}$ 、砷化镓 $\ce{GaAs}$ 、磷化铟 $\ce{InP}$ .
4. 六角密排晶格：
...
一个理想晶体是由全同的称作**基元**的结构单元在空间无限重复而构成，基元可以是原子群。
...
基本平移矢量/基矢：各个方向上平移周期的基本矢量。从一个格点出发的基矢只能指向另一个格点，且不能穿过其它格点。
...
**单胞（惯用晶胞）**也是点阵中完全平移覆盖单元，并能体现旋转对称性。约定俗成地，单胞一般是立方体，所以晶格常数 = 惯用晶胞边长。

魏格纳-塞茨原胞：与相邻原子垂直平分面围成的原胞，是另一种通用的求原胞的方法。
...
1. 边方向 $\langle 100\rangle$ ，有6个。
2. 面对角线方向 $\langle 110\rangle$ ，有12个。
3. 体对角线方向 $\langle 111\rangle$ ，有8个。
...
!!! Note
    立方晶系有 3 种布拉菲格子，即为常见的简单立方、体心立方、面心立方。
...
第 $n+1$ 布里渊区：从原点出发经过 $n$ 个中垂面(或中垂线)才能到达的区域（ $n$ 为正整数）。穿过交点的时候也要算交叉面的数量。**不同布里渊区的面积/体积相等**。
...
金刚石的 $\{111\}$ 晶面为双层原子，原子层面间距之比为 $1:3$ ，但是**晶面一定是等距的**！
