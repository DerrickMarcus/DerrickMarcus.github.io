...
硬件结构实现 1：硬件有一个计数器 C(足够长，例如64位)，每条指令执行后自动增 1。每个页表项有一个能够容纳计数器值的域。每次访问内存后，当前 C 值被复制到被访问页面的页表项中。页表项的计数值最小的页面就是最近最少使用的。

硬件结构实现 2：对于有 n 个页框的机器，LRU 硬件可以维护一个 $n\times n$ 矩阵，初值为 0。访问页框 k 时，先把 k 行的位都置1，再把 k 列的位都清 0。任何时刻，二进制值最小的行就是最近最少使用的。
...
!!! note "虚拟时间"
    一个进程从它开始执行到当前时刻所实际使用的 CPU 时间总数称为当前虚拟时间。
