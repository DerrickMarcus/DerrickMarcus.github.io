...
!!! info "Info"
    ...
    此复习笔记参考2024春季学期清华大学计算机系《操作系统》谌卫军老师课程，《操作系统》第二版（编著：谌卫军，清华大学出版社。）
...
1. 内存的分配：计算一个进程所需要的页面数N，并查看位示图，看是否还有N个空闲页面；若有，则申请一个页表，其长度为N，并把页表的起始地址填入PCB；分配N个空闲物理页面，将其编号填入页表；修改位示图（0-->1，空闲页面数－N）
