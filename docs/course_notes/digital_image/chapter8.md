...
4. PNG 8支持两种不同的透明形式，索引透明和Alpha透明。
5. PNG 24不支持透明。
6. PNG 32在24位基础上增加了8位透明通道，因此可展现256级透明程度。
...
3. 与霍夫曼方法不同，算术编码不需要将每个信源符号转换为整数个码字（即1次编1个符号）。与其它熵编码方法不同的地方在于，其他的熵编码方法通常是把输入的消息分割为符号，然后对每个符号进行编码，而算术编码是直接把**整个输入的消息编码为一个数**，一个满足 $0.0\le n<1.0$ 的小数 n。
