...
实际应用中，这两种方法各有优缺点，可以考虑把**边缘法和区域法结合**起来进行图像分割。
...
3. 对梯度幅值进行非极大值抑制(NMS)，细化边缘。非极大值抑制首先要指定边缘法线的许多离散方向，并为每个方向定义角度范围。如对于水平边缘法线，可定义其方向角范围为 −157.5° 至 +157.5° 和 -22.5° 至 +22.5°。可扩展以定义水平、垂直、+45°、−45°四种边缘法线的离散方向。对某一个像素点，找到最接近 $\alpha(x,y)$ 的方向 $d_k$ ，若该点梯度幅值 $M(x,y)$ 大于沿 $d_k$ 方向上的两个邻居，则 $g_N(x,y)=M(x,y)$ 幅值保持，否则置 $g_N(x,y)=0$ 抑制。由此得到非极大值抑制后的图像 $g_N(x,y)$ 。
4. 双阈值算法检测与边缘连接： $g_{NH}(x,y)=g_N(x,y)\geq T_H, G_{NL}(x,y)=g_N(x,y)\geq T_L$ 。
...
Hough 变换检测的直线并不一定可靠，受到噪声、分辨率、变换参数等因素影响。
...
是分析每一个边缘点 (x,y) 的一个邻域(3x3，5x5)内像素的梯度特性（如强度、方向），然后依据事先预定的准则，连接相似的边缘点，形成由共同满足准则的像素组成的一条边缘。
...
1. 选择一个阈值 T 的初始值。
2. 用 T 分割图像，获得2组像素 G1（灰度大于 T）和G2（灰度小于 T）。
3. 计算区域 G1,G2 中所有像素平均灰度 g1,g2 。
4. 计算新阈值 $T=(g_1 + g_2)/2$
5. 重复步骤2至4，直到逐次迭代所得的 T 值之差小于某参数 T0 。

大津算法（最大类间方差法）。思想：类间方差最大，类内方差最小。判决准则 $\eta(t) = (\sigma_T^2 - \sigma_\omega^2)/\sigma_T^2$ ，最佳阈值t为（等效于使类间方差最大的阈值t） $t^*=\underset{0\leq t\leq L-1}{\argmax}\ \eta(t)$ 。

自适应阈值算法。划分子图像，不同子图像使用不同阈值。由于用于每个像素的阈值，取决于像素在子图像中的位置，所以这类阈值处理是自适应的。重点在划分图像，可以与大津算法、迭代阈值法等结合使用。
...
| 自适应阈值 | 适用于当目标与背景的大小比例悬殊时（例如受光照不均、反光或背景复杂等因素影响）而使类间方差准则函数可能呈现双峰或多峰情况 | 其对于子区域分割是的标准要求较高，只有划分出适宜的子区域，才可以得到较好的分割结果。 |
...
- 采用灰度值的测地线距离：如点 A 到 $C_h(M_1)$ 的测地线距离 $d_{11} + d_{12}$ ，大于点 A 到 $C_h(M_1)$ 的测地线距离 $d_2$ ，因此将点 A 加入到 $C_{h+1}(M_2)$ 中。
...
闭运算： $A\cdot B=(A\oplus B)\minus B$ ，**先膨胀后腐蚀**。A是闭运算结果的子集。闭运算可以填充比结构元素小的缺口或孔，搭接短的间断，起到连通作用。
...
处理策略：（对于**线性独立操作**，两种方法处理结果相同，例如邻域平均）

1. 分别处理彩色图像的每一个分量 $f_r,f_g,f_b$ ，然后合成。
2. 将彩色像素 $f(x,y)$ 看作 3x1 向量，直接对彩色向量处理。
...
彩色分层增强：突出图像中特殊的彩色区域。不能采用各通道灰度切分增强的方法，因为彩色像素是一个 n 维向量。
