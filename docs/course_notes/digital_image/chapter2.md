...
色度图上横纵坐标分别为 r,g 。外切三角形横跨二、四象限，色系数可能为负数。色系数为负数的意义是，将某一种原色与待匹配单色光混合，与另外两个原色的混合达到匹配 $bB+C=rR+gG$ 。
...
4. 接任两端点的直线上的各点表示将这两端点所代表的彩色相加可组成的一种新彩色。
5. 过 W 点直线端点的两彩色为互补色。
6. 三角形包含由三顶点可组成的所有可能的颜色组合。
...
注意，这里的 R,G,B 分量取值范围为常见的 [0, 255]，且相互独立，与前述色彩空间中色度系数 r,g,b 不同，因为它们具有相加为1的约束。只有做归一化之后，才相同，归一化 $r=R/(R+G+B)$ 后得到颜色三角形，即为立方体三个相邻顶点连接的三角形，三角形上面的点满足 $r+g+b=1$ 。
...
光强(luminous intensity)：点光源 Q 沿方向 r 的发光强度 I，定义为沿此方向上单位立体角内发出的光通量 $I=\dfrac{\mathrm{d}\Phi}{\mathrm{d}\Omega}$ ，单位坎德拉 cd。

亮度(brightness)：表示发光面明亮程度，指发光表面在指定方向的发光强度与垂直在指定方向的发光面的面积之比 $B=\dfrac{\mathrm{d}\Phi}{\mathrm{d}\Omega \mathrm{d}S \cos\theta}$ ，单位尼特 nit。

照度(illuminance)：一个被光线照射的物体表面上的照度，定义为照射在单位面积上的光通量 $E=\dfrac{\mathrm{d}\Phi}{\mathrm{d}S}$ ，单位勒克斯 lx。
...
对于 M×N，灰度级为 $L=2^k$ 的数字图像，M,N 为整数，出于处理、存储等硬件考虑，灰度级取值是2的整数次幂。灰度图片的比特数为 MNk，彩色图像比特数为 3MNk。
...
RGB 只是告诉我们颜色可由红、绿、蓝三个分量混合得到，但并未规定具体的红、绿、蓝的颜色究竟指哪三个具体颜色（色坐标）。不同设备的“红色”可能并不相同。因此，单纯的RGB模型无法严格定义颜色的绝对含义。
