...
之前的推导中我们知道，在只有一帧图像+位姿的情况下，缺少深度信息，是无法确定人员在地图中的三维坐标的。但是如果我们做出如下假设：**场景中人员始终静止站在地面上**，可以转化为条件：也就是人员脚部的地图坐标的高度 $Z_W=0$ .

!!! warning
    使用地面 $Z_W=0$ 假设的前提是，建图节点输出的全局地图中，地面平面就是 $Z=0$ 。如果全局地图是以 IMU 初始时刻（与重力对其）的水平面定义的，那么就不是 $Z_W=0$ ，可能形如 $Z_W=-0.5$ .
...
$$
\mathbf{R} = \mathbf{R}_{LC}\left(\mathbf{R}_{LW}\right)^T, \mathbf{t}=\mathbf{t}_{LC}-\mathbf{R}\cdot\mathbf{t}_{LW}
$$
...
$$u=\frac{a_{11}X_W+a_{12}Y_W+a_{14}}{a_{31}X_W+a_{32}Y_W+a_{34}} ,\quad
v=\frac{a_{21}X_W+a_{22}Y_W+a_{24}}{a_{31}X_W+a_{32}Y_W+a_{34}}$$
